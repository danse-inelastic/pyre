...
class Parser(xml.sax.ContentHandler):
    ...
    def endElement(self, name):
        ...
        try:
            ...
            filename = getattr(l, 'source', None)
            if filename is None:
                filename = getattr(l, 'filename', None)
            error.log("{0!s}: line {1!s}, column {2!s}: {3!s}".format(filename, l.line, l.column, text))
    ...
    def processingInstruction(self, target, data):
        ...
        journal.firewall("pyre.xml.parsing").log(
            "processingInstruction: target={{0!s}}, data={{1!s}}".format(target, data)
            )
